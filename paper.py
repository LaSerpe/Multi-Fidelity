...
for iDataRandomization in range(NdataRandomization):
	...
	for nn in range(len(Nobs_array)):
		...
		for iOrdering in range(nOrdering):	
			...
			MF_performance[-(nOrdering-iOrdering)][nn].kernel_param		= np.copy( Mfs[-1].kernel );
			...
			LG_performance[-(nOrdering-iOrdering)][nn].kernel_param		= np.copy( MGs[-1].kernel );
			...
			SF_performance[-(nOrdering-iOrdering)][nn].kernel_param		= np.copy( GP_single.kernel );
	...
	string_save = 'FIGURES/PAPER_PRELIMINARY/' + str(iDataRandomization) + '_' + Mode + '_' + Mode_Opt + '_';
...
for j in range(len(Nobs_array)):
	print("N of T points: " + str(Nobs_array[j]));
	print("MF");
	for i in MF_performance:
		print(i[j].regression_param, i[j].score)
	print("LG");
	for i in LG_performance:
		print(i[j].regression_param, i[j].score)
	print("SF");
	for i in SF_performance:
		print(i[j].regression_param, i[j].score)
