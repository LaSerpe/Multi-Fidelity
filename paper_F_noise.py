...
import subprocess
command = ['rm PRELIMINARY_PAPER_F/*']
...
for iDataRandomization in range(NdataRandomization):
		...
		for iOrdering in range(nOrdering):	
			...
			for Nm in model_order[iOrdering]:
				if not Mfs: 
					...
					Mfs[-1].fit(Train_points[Nm].reshape(-1, 1), observations[Nm][:, 0].reshape(-1, 1), observations[Nm][:, 1].reshape(-1, 1), Opt_Mode= Mode_Opt, LASSO=LASSO);
				else:
			...
			for Nm in model_order[iOrdering]:
				if not MGs: 
					...
					MGs[-1].fit(Train_points[Nm].reshape(-1, 1), observations[Nm][:, 0].reshape(-1, 1), observations[Nm][:, 1].reshape(-1, 1), Opt_Mode= Mode_Opt, LASSO=LASSO);
				else:
			...
			GP_single.fit(Train_points[-1].reshape(-1, 1), observations[-1][:, 0].reshape(-1, 1), observations[Nm][:, 1].reshape(-1, 1), Opt_Mode= Mode_Opt, LASSO=LASSO);
	...
	string_save = 'PRELIMINARY_PAPER_F/' + str(iDataRandomization) + '_' + Mode + '_' + Mode_Opt + '_';
...
for j in range(len(Nobs_array)):
	for iOrdering in range(nOrdering):
		f_IR = open('PRELIMINARY_PAPER_F/regression_params_IR_' + Mode + '_' + Mode_Opt + '_n'+ str(Nobs_array[j]) + '_o' + str(iOrdering) + '.dat', 'w')
		f_SR = open('PRELIMINARY_PAPER_F/regression_params_SR_' + Mode + '_' + Mode_Opt + '_n'+ str(Nobs_array[j]) + '_o' + str(iOrdering) + '.dat', 'w')
		f_SF = open('PRELIMINARY_PAPER_F/regression_params_SF_' + Mode + '_' + Mode_Opt + '_n'+ str(Nobs_array[j]) + '_o' + str(iOrdering) + '.dat', 'w')
...
for iOrdering in range(nOrdering):
	string_save = 'PRELIMINARY_PAPER_F/' + Mode + '_' + Mode_Opt + '_o' + str(iOrdering) + '_';
	if LASSO:      string_save+= 'LASSO_';
	if Matching:      string_save+= 'matching_';
	if Nested:        string_save+= 'nested_';
	if Equal_size:    string_save+= 'equal_';
